...
- **Non-blocking Design** - Uses SmolPaws scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. SmolPaws already set up and running
...
Add Parallel AI MCP servers after the smolpaws server:
```typescript
const mcpServers: Record<string, any> = {
  smolpaws: ipcMcp
};
...
3. Create a polling scheduled task using `schedule_task`:
   ```
   ...
   3. Send results to user with send_message
   4. Use complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo smolpaws-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/smolpaws.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.smolpaws`
