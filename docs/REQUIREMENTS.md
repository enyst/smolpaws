...
SmolPaws gives you the core functionality without that mess.
...
- Trigger: `@smolpaws` prefix (case insensitive), configurable via `ASSISTANT_NAME` env var
...
- Custom `smolpaws` MCP server (inside container) provides scheduling tools
...
- **Trigger**: `@smolpaws` (case insensitive)
...
**SmolPaws** - A reference to Clawdbot (now OpenClaw).
