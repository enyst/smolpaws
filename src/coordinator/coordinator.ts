/**
 * Message Work Coordinator — the ADR §4 interface.
 *
...
export interface CoordinatorOptions {
  ...
  /** What counts as deliverable (ADR D1). Defaults to explicit send_message action events. */
...
export class MessageWorkCoordinator {
  ...
  /**
   * Resolve a lane to its conversation binding, ensuring the conversation exists in agent-server before
   * intake is accepted (crash matrix: mapping is durable first, conversation is then ensured, then the
   * `conversation_ready` flag is set). Idempotent.
   */
  ...
  /**
   * Durably accept one inbound platform message as intake work. Idempotent on the platform message id;
   * a webhook/poll replay returns the existing row. Does NOT append yet — integration is a separate,
   * claimable step so ordering and retries are enforced by the queue.
   */
  ...
  /**
   * Integrate the next ready intake lane-head: claim it, append the deterministic user event with
   * `run=true`, and settle. A lost append response is safe because the event id is deterministic and the
   * append is idempotent (ADR §8). Returns what happened so a worker loop can decide whether to continue.
   */
