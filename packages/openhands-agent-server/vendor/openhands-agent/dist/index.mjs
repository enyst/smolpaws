import { randomUUID, createHash, randomBytes, verify, createPublicKey } from 'crypto';
import { z } from 'zod';
import { execFile, exec, spawnSync, spawn } from 'child_process';
import { promisify, format } from 'util';
import { readFile, stat, readdir, mkdir, open, unlink, access, writeFile, copyFile } from 'fs/promises';
import path2, { basename, dirname, extname, relative, join, resolve, sep, posix, isAbsolute } from 'path';
import { mkdirSync, existsSync, writeFileSync, readFileSync, statSync, openSync, rmSync, closeSync, readdirSync, unlinkSync, chmodSync, renameSync } from 'fs';
...
var llmProfileIdSchema = z.string().regex(LLM_PROFILE_ID_PATTERN);
var llmProviderIdSchema = z.string().min(1).regex(/^[A-Za-z0-9._-]+$/u);
var openAiApiModeSchema = z.union([z.literal("chat_completions"), z.literal("responses")]);
var reasoningEffortSchema = z.union([z.literal("low"), z.literal("medium"), z.literal("high")]);
var reasoningSummarySchema = z.union([z.literal("auto"), z.literal("concise"), z.literal("detailed")]);
var promptCacheRetentionSchema = z.union([z.literal("24h"), z.literal("disabled")]);
var anthropicCacheTtlSchema = z.enum(["5m", "1h"]);
...
var redactedThinkingBlockSchema = z.object({
...
var contentSchema = z.union([textContentSchema, imageContentSchema]);
var messageToolCallSchema = z.object({
  id: z.string(),
...
var rawMessageSchema = z.object({
  ...
  content: z.union([z.string(), z.array(contentSchema), z.null()]).default([]).transform((content) => {
    if (content === null) {
      return [];
    }
...
var condensationSchema = eventObject({
  ...
  forgotten_event_ids: z.union([z.set(z.string()), z.array(z.string())]).transform((ids) => ids instanceof Set ? ids : new Set(ids)),
  llm_response_id: z.string().nullable().default(null)
});
...
var hookEventTypeSchema = z.union([
...
var llmConvertibleEventSchema = z.discriminatedUnion("kind", [
  systemPromptEventSchema,
  messageEventSchema,
  actionEventSchema,
  observationEventSchema,
...
var Skill = class {
  ...
  static async load(path3, skillBaseDir, strict = true) {
    const fileContent = await readFile(path3, "utf8");
    if (basename(path3).toLowerCase() === "skill.md") {
...
async function loadAgentSkill(path3, fileContent, strict) {
  ...
  const directoryName = basename(dirname(path3));
  const name = stringValue(parsed.metadata.name) ?? directoryName;
  if (strict && !isValidAgentSkillName(name)) {
    throw new Error(`Invalid skill name '${name}'`);
  }
  const resources = await discoverSkillResources(dirname(path3));
...
function loadLegacySkill(path3, fileContent, skillBaseDir) {
  const thirdPartyName = thirdPartySkillName(basename(path3));
  if (thirdPartyName !== null) {
    return skillSchema.parse({ name: thirdPartyName, content: fileContent, source: path3, trigger: null });
  }
  const parsed = parseFrontmatter(fileContent);
  const derivedName = skillBaseDir === void 0 ? basename(path3, extname(path3)) : stripMarkdownExtension(relative(skillBaseDir, path3));
...
async function existsDirectory(path3) {
  try {
    return (await stat(path3)).isDirectory();
  } catch {
    return false;
  }
}
...
var LocalFileStore = class {
  ...
  constructor(root, options = {}) {
    const expandedRoot = root.startsWith("~") ? path2.join(process.env.HOME ?? "", root.slice(1)) : root;
    this.root = path2.resolve(path2.normalize(expandedRoot));
  ...
  getFullPath(filePath) {
    ...
    const fullPath = path2.resolve(path2.normalize(path2.join(this.root, toPosixPath(relativePath))));
    const relativeToRoot = path2.relative(this.root, fullPath);
    if (relativeToRoot.startsWith("..") || path2.isAbsolute(relativeToRoot)) {
  ...
  write(filePath, contents) {
    ...
    mkdirSync(path2.dirname(fullPath), { recursive: true });
...
var InMemoryFileStore = class {
  files;
  instanceId = randomUUID().replace(/-/gu, "");
  locks = /* @__PURE__ */ new Set();
  ...
  getAbsolutePath(filePath) {
    return path2.join(tmpdir(), `openhands_inmemory_${this.instanceId}`, filePath);
  }
};
...
async function getGitDiff(filePath, ref) {
  ...
  const info = await stat(path3).catch(() => null);
  if (info === null) {
...
function toPosixPath2(path3) {
  return path3.split(sep).join(posix.sep);
}
...
var InstallationMetadata = class _InstallationMetadata {
  ...
  static metadataPath(installedDir) {
    return join(installedDir, _InstallationMetadata.metadataFilename);
  }
  static async loadFromDir(installedDir) {
    try {
      const raw = JSON.parse(await readFile(_InstallationMetadata.metadataPath(installedDir), "utf8"));
      if (isRecord4(raw)) {
  ...
  async saveToDir(installedDir) {
    ...
    await writeFile(path3, `${JSON.stringify({ extensions: this.extensions }, null, 2)}
`);
  }
...
var recordSchema2 = z.record(z.string(), z.unknown());
var hookEventSchema = z.object({
...
var HookConfig = class _HookConfig {
  ...
  static async load(options = {}) {
    ...
    return new _HookConfig(JSON.parse(await readFile(path3, "utf8")));
  }
  isEmpty() {
  ...
  async save(path3) {
    await mkdir(dirname(path3), { recursive: true });
    await writeFile(path3, JSON.stringify(this.toJSON(), null, 2));
  }
  toJSON() {
...
function getCredentialsDir() {
  ...
  return join(resolve(INITIAL_CWD2, expanded), "auth");
}
var CredentialStore = class {
  ...
  directory;
  get credentialsDir() {
    mkdirSync(this.directory, { recursive: true, mode: 448 });
    if (process.platform !== "win32") chmodSync(this.directory, 448);
...
async function login(auth, options) {
  if (options.authMethod === "device_code") {
    ...
    while (performance.now() < deadline) {
      ...
      await setTimeout$1(Math.min(device.interval * 1e3, Math.max(0, deadline - performance.now())));
    }
...
var openAIChatToolCallSchema = z.object({
  id: z.string(),
...
var openAIResponsesContentItemSchema = z.union([openAIResponsesOutputTextSchema, z.object({ type: z.string() }).passthrough()]);
var openAIResponsesMessageItemSchema = z.object({
  type: z.literal("message"),
  role: z.literal("assistant").default("assistant"),
  content: z.array(openAIResponsesContentItemSchema).default([])
}).passthrough();
var openAIResponsesReasoningSummaryItemSchema = z.object({
  type: z.string().default("summary_text"),
  text: z.string().default("")
}).passthrough();
var openAIResponsesReasoningContentItemSchema = z.object({
  type: z.string().default("reasoning_text"),
  text: z.string().nullable().default(null),
  content: z.string().nullable().default(null)
}).passthrough();
var openAIResponsesReasoningItemSchema = z.object({
...
var openAIResponsesFunctionCallItemSchema = z.object({
  type: z.literal("function_call"),
  id: z.string().nullable().default(null),
  call_id: z.string(),
  name: z.string(),
...
var openAIResponsesOutputItemSchema = z.union([
...
var criticModeSchema = z.union([z.literal("finish_and_message"), z.literal("all_actions")]);
var profileVerificationSettingsSchema = z.object({
...
var agentProfileSchema = z.union([openHandsAgentProfileSchema, acpAgentProfileSchema]);
...
var openHandsAgentSettingsSchema = z.object({
...
async function loadAgentsFromDir(agentsDir) {
  ...
  for (const entry of (await readdir(agentsDir)).sort()) {
    const path3 = join(agentsDir, entry);
    if (skipFiles.has(entry) || extname(entry).toLowerCase() !== ".md" || await isDirectory(path3)) {
      continue;
    }
    try {
...
var finishActionSchema = z.object({
  message: z.string().describe("Final message to send to the user.")
}).strict();
var thinkActionSchema = z.object({
...
var updateTaskActionSchema = z.object({
...
var sendMediaActionSchema = z.object({
...
var fileEditorObservationSchema = baseToolObservationSchema.extend({ command: z.enum(["view", "create", "str_replace", "insert", "undo_edit"]), path: z.string().nullable().default(null), prev_exist: z.boolean().default(true), old_content: z.string().nullable().default(null), new_content: z.string().nullable().default(null) }).strict();
var FileEditorExecutor = class {
  ...
  resolvePath(path3) {
    ...
    if (this.workspaceRoot !== null && !(resolved === this.workspaceRoot || resolved.startsWith(`${this.workspaceRoot}${sep}`))) throw new Error(`Path escapes workspace: ${path3}`);
    return resolved;
  }
  async view(path3, action) {
    ...
    const numbered = numberLines(await readFile(path3, "utf8"), action.view_range);
  ...
  async create(path3, action) {
    ...
    await mkdir(dirname(path3), { recursive: true });
    await writeFile(path3, action.file_text);
  ...
  async undo(path3, action) {
    ...
    const oldContent = await readFile(path3, "utf8").catch(() => "");
    await writeFile(path3, previous);
...
var globActionSchema = z.object({ pattern: z.string(), path: z.string().nullable().default(null) }).strict();
var globObservationSchema = baseToolObservationSchema.extend({ files: z.array(z.string()).default([]), pattern: z.string(), search_path: z.string(), truncated: z.boolean().default(false) }).strict();
...
var grepActionSchema = z.object({ pattern: z.string(), path: z.string().nullable().default(null), include: z.string().nullable().default(null), max_results: z.number().int().positive().default(100) }).strict();
var grepMatchSchema = z.object({ file: z.string(), line: z.number(), text: z.string() }).strict();
var grepObservationSchema = baseToolObservationSchema.extend({ matches: z.array(grepMatchSchema), pattern: z.string(), search_path: z.string(), truncated: z.boolean().default(false) }).strict();
...
var taskItemSchema = z.object({ title: z.string(), notes: z.string().default(""), status: z.enum(["todo", "in_progress", "done"]).default("todo") }).strict();
var taskTrackerActionSchema = z.object({ command: z.enum(["view", "plan"]).default("view"), task_list: z.array(taskItemSchema).default([]) }).strict();
var taskTrackerObservationSchema = baseToolObservationSchema.extend({ command: z.enum(["view", "plan"]), task_list: z.array(taskItemSchema).default([]) }).strict();
var TaskTrackerExecutor = class {
  ...
  async saveTasks() {
    ...
    await writeFile(join(this.saveDir, "TASKS.md"), formatTasks(this.taskList));
  }
};
...
var browserActionSchema = z.object({ command: z.enum(["navigate", "get_state", "click", "type", "scroll", "back"]), url: z.string().nullable().default(null), index: z.number().int().nullable().default(null), text: z.string().nullable().default(null), direction: z.enum(["up", "down"]).default("down") }).strict();
...
var LocalWorkspace = class {
  ...
  resolvePath(path3) {
    return isAbsolute(path3) ? resolve(path3) : resolve(this.workingDir, path3);
  }
};
